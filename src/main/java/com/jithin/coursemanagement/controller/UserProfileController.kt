...
import com.jithin.coursemanagement.converters.UserProfileToUserProfileResponseConverter
...
class UserProfileController {
    ...
    @Autowired
    private lateinit var mUserProfileToUserProfileResponseConverter: UserProfileToUserProfileResponseConverter
    ...
    @GetMapping("/all")
    fun getProfiles(principal: Principal): ResponseEntity<Any> {

        val user =
            userService.findUserByEmail(principal.name).orElseThrow { UnAuthenticatedUser("unauthenticated user") }

        val uProfiles = profileService.getProfileOfUser(user.id)
        val cProfiles = uProfiles.map { item -> mUserProfileToUserProfileResponseConverter.convert(item) }
        return ResponseEntity.ok(cProfiles)

    }

    @GetMapping("/detail")
    fun getProfileById(principal: Principal): ResponseEntity<Any> {
        val user =
            userService.findUserByEmail(principal.name).orElseThrow { UnAuthenticatedUser("unauthenticated user") }
       val profile = mUserProfileToUserProfileResponseConverter.convert(user.userProfile)
